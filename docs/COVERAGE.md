...
| L2 T1, normal sampling | Partial | [NormalSampling](../LectureNotes/NormalSampling.lean): `normal_sampleMean` proves part (1); `normal_sampleMean_independent_sampleVariance` proves part (3), through independence from the full residual vector. Parts (2), chi-square variance law, and (4), Student-t law, remain missing. |
...
| L1 moments and covariance | Integral expectation, variance, covariance identities, independence implies zero covariance, zero variance implies constancy almost everywhere, covariance Cauchy–Schwarz bound | Correlation equality characterization; every distribution-specific moment table and example. |
| L1 distribution theory | Normal laws used through Mathlib | Full distribution catalog, density/CDF transformations, conditional multivariate-normal formulas, normal quadratic-form laws and projection-rank proof. |
| L2 statistics and simulation | Measurable statistic/estimator types, integral-based unbiasedness; sample mean, sample variance, unbiased sample-variance lemma; finite population and Horvitz–Thompson calculations | General sampling-distribution examples, Monte Carlo algorithms, empirical quantile/order-statistic proofs, F-statistic law. |
...
| L5 risk and examples | MSE/bias/variance, exact shrinkage risk, exponential-family factorization, finite positive sufficiency and minimal sufficiency | General loss/risk decision theory; every strict risk comparison; normal variance-estimator risk and fourth-moment CLT examples; concrete sufficient/minimal statistics and Rao–Blackwell counting examples. |
